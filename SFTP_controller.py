...
class sftp_controller:  
	...
	def connect_to(self, Host, Username='', Password='', Port=22, Keyfile=None, Passphrase=None):
		...
		try:
			if Keyfile:
				print("Key")
				self.ssh.connect(Host,port=Port,username=Username,key_filename=Keyfile,passphrase=Passphrase)
			else:
				print("No key")
		...
		self.ftp.cwd = self.ftp.chdir
		try:
			self.ftp.cwd('/home/' + Username)
		except:
			self.ftp.cwd('/')
	...
	def get_detailed_file_list(self, ignore_hidden_files_flag = False):
		...
		for attr in self.ftp.listdir_attr():
			if(self.hidden_files or str(attr).split()[8][0]!= '.') or ignore_hidden_files_flag == True:
	...
	def chown(self, filename, uid, gid):
		self.ftp.chown(filename, uid, gid)
	...
	def move_dir(self, rename_from, rename_to, status_command, replace_command):
		if(self.is_there(rename_to)):
			if(replace_command(rename_from, 'File/Folder exists in destination folder')):
	...
	def upload_file(self, file_name, file_size, status_command, replace_command):
		...
		if(self.is_there(file_name)):
			if(not replace_command(file_name, 'File exists in destination folder')):
				return
		#Try to upload file
		try:
	...
	def download_file(self, ftp_file_name, file_size, status_command, replace_command):
		...
		if(isfile(ftp_file_name)):
			if(not replace_command(ftp_file_name, 'File exists in destination folder')):
				return
